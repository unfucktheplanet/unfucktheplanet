...
Devise.setup do |config|
  # Specify a sender address which will be shown in Devise::Mailer - will be
  # overwritten if you use a default "from" parameter in your mailer class.
  ...
  # Configure any authentication mechanism. Configure which keys are used when
  # authenticating a user. The default is just :email. You can configure it to
  # use [:username, :subdomain], so for authenticating a user, both parameters
  # are required. Remember that those parameters are used only when
  # authenticating and not when retrieving from session. If you need
  # permissions, you should implement that in a before filter.
  #
  ...
  # Tell if authentication through HTTP Basic Auth is enabled. Off by default.
  # It can be set to an array that will enable http authentication only for
  # the given strategies, for example, `config.http_authenticatable = [:token]`
  # will enable it only for token authentication.
  ...
  # config.pepper = "d075e6a8d9309500ab25e08b84ee56ae73fdd1a3ce976dbc4db01624a5b7d99eb8d54175cd19575a79af2b4bb4b9976728013c3ffb3db214ac92d1c0dd2d1bc2"
  ...
  # as initial account confirmation) to be applied. Requires an additional
  ...
  #
  #   :failed_attempts = locks after a number of failed sign-in attempts
  #   :none            = no lock strategy - handle locking by yourself
  #
  ...
  #   :none  = no unlock strategy - handle unlocking by yourself
  #
  ...
  if File.exists? "#{Rails.root}/config/omniauth.yml"
    fb     = YAML.load_file("#{Rails.root}/config/omniauth.yml")[Rails.env]
    appid  = fb["appid"]
    secret = fb["secret"]
  end

  unless appid.present? && secret.present?
    raise "Facebook 'APPID' and 'SECRET' not specified"
  end
